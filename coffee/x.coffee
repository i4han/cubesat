...
x.isString    = (v) -> 'string'    is typeof v 
...
x.isLower = (v) -> x.isString(v) and v.toLowerCase() is v
x.isUpper = (v) -> x.isString(v) and v.toUpperCase() is v
x.isHTMLTag      = (o) -> o instanceof HTML.Tag
x.isBlazeView    = (o) -> o instanceof Blaze.View 
x.isBlazeElement = (o) -> x.isHTMLTag(o) or x.isBlazeView(o) or x.isString(o)
x.isLookup          = (o) -> x.isObject(o) and x.isString(o.name) and o.name[..6] is 'lookup:'
x.isBlazeAttr       = (o) -> x.isObject(o) and not x.isLookup(o) and not x.isBlazeElement o
x.isAttrPartKey     = (k) -> x.isString(k) and '$' is k[0]
...
x.eachKeys   = (obj, f)    -> x.keys(obj).forEach f
...
x.array  = (a) -> 
   args = [].slice.call(arguments)
   if x.isArray(a) then (if args.length is 1 then  a  else  a .concat args[1..])
   else                 (if args.length is 1 then [a] else [a].concat args[1..])
x.theKey = (obj) -> x.keys(obj)[0]
...
x.fixup = (v) -> switch
   ...
   when x.isFunction v then (if x.isScalar(r = x.return v, @) then r else x.fixup.call @, r)
   when x.isArray    v then v.reduce ((o, w) -> x.assign o, x.fixup.call @, w), {}
   when x.isObject   v then x.reduceKeys v, {}, (o, k) =>
      if '$' is k[0] and k of (Parts or (Parts = exports.Parts?())) then x.assign o, x.fixup.call @, Parts[k].call @, v[k]
      else x.object o, k, (if x.isScalar(r = v[k]) then r else x.fixup.call @, r)
