...
var checkTable, moduleMethod, __NotPxDefaultProperties, __idClassKey, __localTags, __tideEventKey,
...
x.isLower = function(v) {
  return x.isString(v) && v.toLowerCase() === v;
};

x.isUpper = function(v) {
  return x.isString(v) && v.toUpperCase() === v;
};

x.isHTMLTag = function(o) {
  return o instanceof HTML.Tag;
};

x.isBlazeView = function(o) {
  return o instanceof Blaze.View;
};

x.isBlazeElement = function(o) {
  return x.isHTMLTag(o) || x.isBlazeView(o) || x.isString(o);
};

x.isLookup = function(o) {
  return x.isObject(o) && x.isString(o.name) && o.name.slice(0, 7) === 'lookup:';
};

x.isBlazeAttr = function(o) {
  return x.isObject(o) && !x.isLookup(o) && !x.isBlazeElement(o);
};

x.isAttrPartKey = function(k) {
  return x.isString(k) && '$' === k[0];
};

x.isFunctionPartKey = function(k) {
  return x.isString(k) && x.isLower(k[0]);
};

x.maybeMustache = function(v) {
  return x.isString(v) && v.indexOf('{') !== -1 && v.indexOf('}') !== -1;
};

x.maybeHtmlEntity = function(v) {
  return x.isString(v) && v.indexOf('&#' > -1 && v.indexOf(';' > -1));
};
...
x.eachKeys = function(obj, f) {
  return x.keys(obj).forEach(f);
};
...
x.__assign = function(obj, properties) {
  var key, val;
...
x.object = function(obj) {
  ...
  args = [].slice.call(arguments);
  switch (false) {
    case !(x.isObject(obj) && x.isString(args[1])):
      obj[args[1]] = args[2];
      return obj;
    case !(x.isObject(obj) && x.isArray(args[1])):
  ...
  switch (false) {
    ...
    case !x.isFunction(v):
      if (x.isScalar(r = x["return"](v, this))) {
        ...
        return x.fixup.call(this, r);
      }
    case !x.isArray(v):
      return v.reduce((function(o, w) {
        return x.assign(o, x.fixup.call(this, w));
      }), {});
    case !x.isObject(v):
      return x.reduceKeys(v, {}, function(o, k) {
        var Parts;

        if ('$' === k[0] && k in (Parts || (Parts = typeof exports.Parts === "function" ? exports.Parts() : void 0))) {
          return x.assign(o, x.fixup.call(_this, Parts[k].call(_this, v[k])));
        } else {
          return x.object(o, k, (x.isScalar(r = v[k]) ? r : x.fixup.call(_this, r)));
        }
      });
  }
};
    ...
    return (Array(depth).join('    ')) + (function() {
      switch (false) {
        ...
        case '' !== value:
          return key;
        case '' !== key:
          return x.__value(value);
        default:
          return key + ' ' + x.__value(value);
      }
    })();
  }).join('\n');
};
...
x.key2class = function(k) {
  return x.dasherize(k.slice(1));
};

x.key2id = function(k) {
  if (k && moduleMethod.id in this) {
    return this[moduleMethod.id](k);
  } else {
    return '';
  }
};

x.key2attribute = function(k) {
  return x.dasherize(k);
};

x.module = function(name, m) {
  m.name = name;
  m.title = m.label = m.label || m.title || x.capitalize(name);
  m.block = m.block || 'x';
...
x.isModule = function(m) {
  return x.isObject(m) && moduleMethod.id in m && 'label' in m;
};
