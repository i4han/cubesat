...
var Settings, add, api, argv, async, build, build_client_path, build_dir, build_lib_path, build_path, build_public_path, cd, chokidar, client_dir, coffee, coffee_clean, coffee_compile, coffee_ext, coffee_watch, collectExt, command, compare_file, cp, cpdir, create, create_test, cs, cubesat_name, cubesat_package_path, cubesat_path, cwd, directives, dot_cubesat, dot_cubesat_path, dot_sat, dot_sat_path, dotenv, eco, env, error, exec, f, findRoot, func, getVersion, github_file, github_url, gitpass, help, home, https, incVersion, index_basename, index_coffee, index_coffee_path, init, init_settings, install_mobile, isType, jade, lib_dir, lib_files, loadSettings, mcTable, mc_obj, meteor_create, meteor_packages, meteor_packages_removed, meteor_publish, meteor_refresh, meteor_update, mkdir, mobile_config, mobile_config_js, mobile_packages, my_packages, nconf, no_seperator, nocacheRequire, node_modules, npm_publish, npm_refresh, npm_update, ok, options, package_js, package_json, package_paths, packages_dir, path, ps, public_dir, public_files, rePublish, readExports, readWrite, rmdir, run, settings, settings_json, settings_path, site_coffees, site_path, spawn, spawn_command, strOrObj, style_path, stylus, task, tasks, test, test_client_path, test_dir, test_lib_path, test_packages_path, test_path, test_public_path, toObject, toString, toTidy, update_all, version, with_test, write_build, x, __RmCoffee_paths, _meteor_run, _publish, _ref, _ref1,
...
toTidy = function(v, d) {
  if (x.isString(v[d])) {
    ...
    return x.tideValue(x.tideKey(toObject(v[d]), v[x.f.id], __indexOf.call(no_seperator, d) >= 0 ? '' : ' '));
  }
};
...
build = function() {
  settings();
  spawn_command('coffee', (command === 'build' ? '-bc' : '-bcw'), ['-o', build_lib_path, site_coffees.join(' ')], site_path);
  mkdir(build_client_path);
  this.Modules = site_coffees.reduce((function(o, f) {
    return x.extend(o, readExports(add(site_path, f), 'Modules'));
  }), {});
